...
Inject `IBinanceSharedApiClient` when a service needs multiple Binance Shared API surfaces. It exposes `SpotRest`, `UsdFuturesRest`, `CoinFuturesRest`, `SpotSocket`, `UsdFuturesSocket`, `CoinFuturesSocket`. When the operation or transport is selected dynamically, use a typed capability descriptor:
