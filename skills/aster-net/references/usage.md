...
Inject `IAsterSharedApiClient` when a service needs multiple Aster Shared API surfaces. It exposes `SpotRest`, `FuturesRest`, `SpotV3Rest`, `FuturesV3Rest`, `SpotSocket`, `FuturesSocket`, `SpotV3Socket`, `FuturesV3Socket`. When the operation or transport is selected dynamically, use a typed capability descriptor:
