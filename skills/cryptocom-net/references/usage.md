...
Inject `ICryptoComSharedApiClient` when a service needs multiple CryptoCom Shared API surfaces. It exposes `Rest`, `Socket`. When the operation or transport is selected dynamically, use a typed capability descriptor:
...
Inject `ICryptoComRestClient` and `ICryptoComSocketClient`, or follow the consuming project's existing interface pattern. `AddCryptoCom` registers shared REST and socket interfaces from `ExchangeApi.SharedApi`.
